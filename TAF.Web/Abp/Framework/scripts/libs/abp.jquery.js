...
(function ($) {
    ...
    $.extend(abp.ajax, {
        ...
        showError: function (error, useAbpMessage) {
            if (useAbpMessage){
                if (error.details) {
                ...
                switch (jqXHR.status) {
                    case 401:
                        abp.ajax.handleUnAuthorizedRequest(
                            abp.ajax.showError(abp.ajax.defaultError401, userOptions.useAbpMessage),
                            abp.appPath
                        );
                        break;
                    case 403:
                        abp.ajax.showError(abp.ajax.defaultError403, userOptions.useAbpMessage);
                        break;
                    case 404:
                        abp.ajax.showError(abp.ajax.defaultError404, userOptions.useAbpMessage);
                        break;
                    default:
                        abp.ajax.showError(abp.ajax.defaultError, userOptions.useAbpMessage);
                        break;
                }
            }
